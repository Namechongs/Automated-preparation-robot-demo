# 涂层配制系统

基于PyQt5的自动化涂层配制控制系统，通过图形界面与机械臂、泵等设备交互，实现智能配方配制。
## 作者：NameChong 王

## 功能特性

- **智能配方生成**: 基于DeepSeek AI自动推理生成涂层配方
- **多方案支持**: 支持生成多个配制方案供参考选择
- **JSON校验**: 执行前自动校验配方逻辑，拦截机械爪冲突、泵位不匹配等错误
- **自动纠错**: 配方校验失败时自动请求AI修正，最多重试3次，超过则熔断保护
- **自动化执行**: 机械臂、泵、搅拌器协同工作，全流程自动化
- **实时监控**: LED指示灯实时显示各模块运行状态
- **配方保存**: 支持将生成的配方保存为本地JSON文件
- **运行日志**: 自动记录运行日志到 `运行日志/` 目录
- **模拟模式**: 无硬件环境下可进行完整流程测试

## 系统要求

- Python 3.8+
- PyQt5
- OpenAI SDK

## 安装

```bash
pip install PyQt5 openai
```

## 项目结构

```
├── main.py          # 主程序入口，GUI界面与业务逻辑
├── robot.py         # 机械臂控制模块
├── vaildator.py     # JSON配方校验模块
├── main_ui.py       # PyQt5 UI代码（由Qt Designer生成）
├── main.ui          # Qt Designer界面文件
└── dobot_api.py     # 机械臂API（真机连接使用）
```

## 使用说明

### 1. 启动程序

```bash
python main.py
```

### 2. 生成配方

1. 在左侧输入框输入配制需求（如："配制防水涂层，需要耐高温"）
2. 点击 **解析** 按钮
3. 等待AI生成配方方案（支持生成多个方案）
4. 在左侧查看生成的JSON配方

### 3. 执行配方

1. 确认配方内容无误
2. 点击 **开始执行** 按钮
3. 系统自动进行JSON校验，通过后按步骤执行：
   - 机械臂移动到烧杯位置
   - 夹爪抓取烧杯
   - 依次旋转到各泵位置接料
   - 移动到搅拌台进行搅拌
   - 移动到出料口完成出料

### 4. 保存配方

点击 **保存配方** 按钮，将当前配方保存为JSON文件到 `保存的配方/` 目录。

### 5. 模型切换

点击 **切换模型** 按钮在普通模型（deepseek-chat）和推理模型（deepseek-reasoner）之间切换。

## 原料库

系统固定使用4个泵，对应4种原料：

| 泵编号 | 原料 | 比例范围 | 特性 |
|--------|------|----------|------|
| 泵1 | 丙烯酸乳液 | 40%-70% | 必须最后加入 |
| 泵2 | 消光剂+消泡剂 | 5%-15% | 加入后需搅拌 |
| 泵3 | 炭黑分散液 | 10%-25% | - |
| 泵4 | 去离子水 | 10%-30% | - |

单次配制总量不超过100ml。

**JSON格式要求**：materials数组中需包含add_order字段表示加入顺序，数字越小越先加入。

## 配置说明

### 机械臂配置

默认IP地址: `192.168.5.1`（可在 `main.py` 第269行修改）

### 模拟模式

默认启用模拟模式，用于无硬件测试。正式使用时修改 `main.py` 第270行：

```python
self.robot.start(sim=False)  # 关闭模拟，连接真机
```

### API配置

默认使用DeepSeek API，API Key配置在 `main.py` 第18-21行。

## 点位定义

| 点位 | 说明 |
|------|------|
| safe | 机械臂初始安全位置 |
| arm_start | 烧杯正后方等待位 |
| arm_startforward | 烧杯夹取位 |
| arm_startup | 夹取后抬起位 |
| arm_stirup | 搅拌台上方过渡位 |
| arm_stir | 搅拌台放置位 |
| arm_endup | 出料口上方过渡位 |
| arm_end | 出料口放置位 |
| pump_1~4 | 4个泵的出料等待位 |

## 输出示例

系统输出的JSON格式：

```json
{
  "task_name": "任务名称",
  "requirement": "用户需求原文",
  "formula_reasoning": "整体选料依据",
  "plans": [
    {
      "plan_id": 1,
      "plan_name": "方案A：xxx",
      "plan_reasoning": "该方案的具体理由",
      "materials": [
        {"pump_id": 1, "material": "丙烯酸乳液", "total_amount_ml": 60, "add_order": 2},
        {"pump_id": 2, "material": "消光剂+消泡剂", "total_amount_ml": 10, "add_order": 1},
        {"pump_id": 3, "material": "炭黑分散液", "total_amount_ml": 15, "add_order": 3},
        {"pump_id": 4, "material": "去离子水", "total_amount_ml": 15, "add_order": 4}
      ],
      "steps": [
        {"step_id": 1, "action": "move", "target": "safe"},
        {"step_id": 2, "action": "move", "target": "arm_start"},
        ...
      ]
    }
  ]
}
```

**说明**：
- `add_order`: 表示原料加入顺序，数字越小越先加入
- `total_amount_ml`: 该原料的总加入量
- 系统会自动校验比例是否在允许范围内、加入顺序是否正确、是否按要求最后加入等

## 开发说明

### 添加新的点位

编辑 `robot.py` 中的 `POINT` 字典：

```python
'pump_5': [x, y, z, rx, ry, rz]
```

### 添加新的动作类型

在 `robot.py` 的 `execute_step` 方法中添加：

```python
elif action == 'new_action':
    # 处理逻辑
```

### 添加新的原料

编辑 `vaildator.py` 中的 `MATERIAL_CONFIG` 字典：

```python
MATERIAL_CONFIG = {
    1: {"name": "丙烯酸乳液", "ratio_min": 0.40, "ratio_max": 0.70, "solid_content": 1.0, "must_be_last": True, "need_stir_after": False},
    2: {"name": "消光剂+消泡剂", "ratio_min": 0.05, "ratio_max": 0.15, "solid_content": 1.0, "must_be_last": False, "need_stir_after": True},
    5: {"name": "新原料", "ratio_min": 0.05, "ratio_max": 0.20, "solid_content": 0.5, "must_be_last": False, "need_stir_after": False}
}
```

### 新增校验函数

validator.py提供三个独立校验函数：

- `check_ratio(materials, plan_name, total_ml)`: 检查原料比例是否在允许范围内
- `check_add_order(steps, pump_order_map, plan_name)`: 检查加入顺序是否与add_order一致
- `check_must_be_last(steps, pump_order_map, plan_name)`: 检查must_be_last原料是否最后加入

## 许可证

MIT License

## 2026开发日志

- 3.17 - 3.18 实现系统硬件选型，构建系统基本布局图
- 3.19 完成整个系统的系统流程，定义各机械臂姿态与点位
- 3.20 完成系统软件基础GUI设计，利用QT完成，完成多线程，解决按钮卡顿问题 完成json基本格式与llm提示词的设计
- 3.21 系统各部件状态显示功能
- 3.24 - 3.27 完成设备采购与合同签订
- 3.28 完成json校验功能
- 3.30 完成了历史记录功能
- 3.31 完成自动纠错与计数功能，实现智能熔断保护机制
- 4.x 完成配方校验系统升级：新增比例校验、加入顺序校验、必须最后加入校验、搅拌时机校验

## 待完成

- 真机到货后：TCP标定、点位示教、联调测试
- 系统测试扩展功能
- 停止与复位按钮功能实现（当前已注释）
- 泵出料串口指令对接（硬件通信协议）
- 多用户权限管理
- 配方版本控制与回滚功能
- 生产数据统计与分析模块
